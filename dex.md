--[[
	...
	Dex is the ultimate debugging suite designed to help the user debug games and find any potential vulnerabilities.
	
	Agents will receive constant updates, and have a lot of features that are NOT on the
    Final version of Dex, including the powerful Server Output viewer and the 3D Viewer.

    This is a Preview Release. The Full Release build will be Available on the Discord Server.
    (NOTE: Only agents will be able to access the Full Release.)
	
	Discord Server: https://discord.gg/jnXFq2VBgU
]]

-- Main vars
local Main, Explorer, Properties, ScriptViewer, ThreeDViewer, SSPanel, DatastoreEditor, DexHoney, DefaultSettings, Notebook, Serializer, Lib, WelcomeMsg
local API, RMD

game:GetService("ServerScriptService")
game:GetService("ServerStorage")

Instance.new("Folder", game:GetService("ServerScriptService")).Name = "Unavailable on Preview"
Instance.new("Folder", game:GetService("ServerStorage")).Name = "Unavailable on Preview"

-- Default Settings (Cyberpunk Neon Theme by Manus)
DefaultSettings = (function()
	...
	return {
		Explorer = {
			_Recurse = true,
			Sorting = true,
			TeleportToOffset = Vector3.new(0, 5, 0), -- MODIFICADO: Para não ficar preso dentro dos objetos
			ClickToRename = true,
		...
		Theme = {
			_Recurse = true,
			-- Cores Principais (Roxo Escuro e Cinza Azulado)
			Main1 = rgb(24, 24, 100),
			Main2 = rgb(22, 22, 36), -- Um roxo bem escuro, mas não preto

			Outline1 = rgb(40, 35, 60), -- Bordas principais
			Outline2 = rgb(80, 70, 110), -- Bordas de botões
			Outline3 = rgb(30, 25, 50), -- Bordas de caixas de texto
			TextBox = rgb(28, 28, 42),
			Menu = rgb(22, 22, 32),
			ListSelection = rgb(194, 77, 255), -- Cor da seleção (ROXO VIBRANTE)
			Button = rgb(45, 45, 65),
			ButtonHover = rgb(60, 60, 85),
			ButtonPress = rgb(30, 30, 50),
			Highlight = rgb(70, 60, 100),
			Text = rgb(224, 224, 255), -- Texto principal (Branco levemente azulado)
			PlaceholderText = rgb(120, 120, 140),
			Important = rgb(255, 80, 120), -- Vermelho/Rosa para alertas
			ExplorerIconMap = "",
			MiscIconMap = "",
			-- Cores do Editor de Código (Syntax Highlighting)
			Syntax = {
				Text = rgb(210, 210, 230),
				Background = rgb(15, 15, 25), -- Fundo do editor bem escuro
				Selection = rgb(255, 255, 255),
				SelectionBack = rgb(138, 43, 226), -- Roxo para texto selecionado
				Operator = rgb(255, 100, 150), -- Operadores (+, -, *) em rosa
				Number = rgb(255, 220, 100), -- Números em amarelo
				String = rgb(100, 255, 200), -- Strings em verde-água
				Comment = rgb(100, 100, 120), -- Comentários em cinza-azulado
				Keyword = rgb(255, 80, 120), -- Palavras-chave (if, else, for) em rosa choque
				Error = rgb(255, 50, 50),
				FindBackground = rgb(141, 118, 0),
				MatchingWord = rgb(85, 85, 85),
				BuiltIn = rgb(0, 255, 255), -- Funções nativas (print, wait) em CIANO
				CurrentLine = rgb(25, 25, 45),
				LocalMethod = rgb(100, 255, 255), -- Métodos em ciano claro
				LocalProperty = rgb(150, 200, 255), -- Propriedades em azul claro
				Nil = rgb(255, 180, 80), -- Nil e Bool em laranja
				Bool = rgb(255, 180, 80),
				Function = rgb(255, 80, 120), -- 'function' em rosa choque
				Local = rgb(255, 80, 120),
				Self = rgb(255, 80, 120),
				FunctionName = rgb(100, 255, 255), -- Nomes de função em ciano claro
				Bracket = rgb(210, 210, 230)
			},
		}
	}
end)()
...
local EmbeddedModules = {
	...
	The main explorer interface
		...
		local Explorer, Properties, ScriptViewer, ThreeDViewer, SSPanel, DatastoreEditor, DexHoney, Notebook -- Major Apps
		...
		local function initAfterMain()
			...
			ThreeDViewer = Apps.ThreeDViewer
			SSPanel = Apps.SSPanel
			DatastoreEditor = Apps.DatastoreEditor
			DexHoney = Apps.DexHoney
			Notebook = Apps.Notebook
		end

		local function main()
			local Explorer = {}
			local nodes,tree,listEntries,explorerOrders,searchResults,specResults,previewingObjs = {},{},{},{},{},{},{}
			local expanded
			...
			local idCounter,currGlobId = 0, 0
			...
			Explorer.ShowRightClick = function()
				...
				if sList[1].Id and table.find(previewingObjs, sList[1].Id)then
					context:AddRegistered("STOP_PREVIEW")
				else
					context:AddRegistered("3D_PREVIEW")
				end
			...
			Explorer.InitRightClick = function()
				...
				context:Register("3D_PREVIEW",{Name = "3D Preview", OnClick = function()
					local sList = selection.List
					local isa = game.IsA

					currGlobId += 1
					sList[1].Id = currGlobId

					ThreeDViewer.Preview(sList[1].Id, sList[1].Obj)

					table.insert(previewingObjs, sList[1].Id)
				end})

				context:Register("STOP_PREVIEW", {Name = "Stop 3D Preview", OnClick = function()
					local sList = selection.List

					local objIndex = table.find(previewingObjs, sList[1].Id)
					if objIndex then
						table.remove(previewingObjs, objIndex)
						ThreeDViewer.UnPreview(sList[1].Id)
					end
				end})
				context:Register("4D_VIEW",{Name = "4D View", OnClick = function()
					local sList = selection.List
					local isa = game.IsA
					local t = 0
					for i = 1,#sList do
						local node = sList[i]
						if isa(node.Obj,"BasePart") and nodes[node.Obj] then
							t = math.max(t, node.Obj:GetMass())
							break
						end
					end
				end})
...
return search]==]
	...
	local theme = {
    MainColor1 = Color3.fromRGB(24, 24, 38), -- Roxo/Azul bem escuro
    MainColor2 = Color3.fromRGB(18, 18, 28), -- Roxo/Azul ainda mais escuro
    Button = Color3.fromRGB(45, 45, 65)      -- Botão em tom de roxo acinzentado
}
				...
				local createGui = function(self)
					local gui = create({
						{1,"ScreenGui",{Name="Window",ResetOnSpawn = false,}},
			...
			Lib.ContextMenu = (function()
				...
				local function createGui(self)
					local contextGui = create({
						{1,"ScreenGui",{DisplayOrder=1000000,Name="Context",ZIndexBehavior=1,ResetOnSpawn = false,}},
			...
			Lib.BrickColorPicker = (function()
				...
				local function createGui(self)
					local gui = create({
						{1,"ScreenGui",{Name="BrickColor",ResetOnSpawn = false,}},
	...
	ThreeDViewer = function()
		--[[
			3D Viewer App Module
			
			Functional interface for the 3D Viewer.
			Allows the user to preview and explore 3D objects in the workspace.
		]]
		...
		local mouse = nil
		local maxDegree = math.rad(18)

		local mouseDragging = false
		local keyHolding = false

		local currentGlobInput = nil
		...
		local function rotateCamera()
			if previousMousePosition then
				local deltaX = mouse.X - previousMousePosition.X
				local deltaY = mouse.Y - previousMousePosition.Y

				local rotationX = math.rad(deltaY) * maxDegree
				local rotationY = math.rad(deltaX) * maxDegree

				local currentLookAt = camera.CFrame.Position + camera.CFrame.LookVector
				local rotation = CFrame.new(camera.CFrame.Position, currentLookAt)
				rotation = rotation * CFrame.Angles(rotationX, rotationY, 0)
		...
		local function moveCamera(direction)
			...
			elseif direction == "left" then
				directionVector = -camera.CFrame.RightVector
			elseif direction == "right" then
				directionVector = camera.CFrame.RightVector
			end

			camera.CFrame = camera.CFrame + directionVector * 0.5
		end

		local function determineDirection(input)
		...
		local controllerThread = nil

		local function main()
			local ThreeDViewer = {}

			local window, tdPrvwFrame, vpFrame

			local frameNotice, previewingObjs = {}, {}

			local isPreviewing = false

			mouse = service.Players.LocalPlayer:GetMouse()

			local function returnToInitial()
				window:SetTitle("3D Viewer")
				window:Resize(585,450)
				...
				for i, v in pairs(frameNotice) do
					v.Visible = true
				end
			end

			ThreeDViewer.Preview = function(targetId, target)
				if not targetId and not target then return nil end

				if not isPreviewing then
					window:SetTitle("3D Preview")
					window:Resize(585,510)
					...
					vpFrame.Parent = tdPrvwFrame
				end

				local t = target:Clone()

				camera.CFrame = getCameraCFrame(t)

				t.Name = targetId
				t.Parent = vpFrame

				table.insert(previewingObjs, targetId)

				if not isPreviewing then
					if controllerThread == nil then
						controllerThread = coroutine.create(function()
							...
							service.UserInputService.InputEnded:Connect(function(input)
								...
								if input.UserInputType == Enum.UserInputType.Keyboard then
									keyHolding = false
									currentGlobInput = nil
								end
							end)

							service.RunService.RenderStepped:Connect(function()
								if isPreviewing == false then coroutine.yield() end
								...
								if keyHolding then
									determineDirection(currentGlobInput)
								end
							end)
						end)
					end
					...
					coroutine.resume(controllerThread)
				end

				window:Show()
			end

			ThreeDViewer.UnPreview = function(targetId)
				if not targetId then return nil end

				local targetIndex = table.find(previewingObjs, targetId)

				vpFrame[targetId]:Destroy()
				table.remove(previewingObjs, targetIndex)

				if #previewingObjs == 0 then
					returnToInitial()
				end
			end

			ThreeDViewer.Init = function()
				window = Lib.Window.new()
				window:SetTitle("3D Viewer")
				window:Resize(585,450)
				window:SetResizable(false)
				ThreeDViewer.Window = window

				tdPrvwFrame = Instance.new("Frame")
				tdPrvwFrame.BackgroundTransparency = 1
				tdPrvwFrame.ClipsDescendants = true
				tdPrvwFrame.Size = UDim2.new(1, 0, 1, 0)
				tdPrvwFrame.Position = UDim2.new(0, 0, 0, 0)
				tdPrvwFrame.Parent = window.GuiElems.Content

				local fSign = Instance.new("TextLabel", tdPrvwFrame)
				fSign.Position = UDim2.new(0, 294, 0, 17)
				fSign.Text = "3D Viewer"
				fSign.FontFace = Font.new("rbxasset://fonts/families/SourceSansPro.json", Enum.FontWeight.Bold, Enum.FontStyle.Normal)
				fSign.TextSize = 27
				fSign.TextColor3 = Color3.new(1, 1, 1)

				local fSign1 = Instance.new("TextLabel", tdPrvwFrame)
				fSign1.Position = UDim2.new(0, 294, 0, 43)
				fSign1.Text = "Welcome to Agency's powerful utility, 3D Viewer!"
				fSign1.FontFace = Font.new("rbxasset://fonts/families/SourceSansPro.json", Enum.FontWeight.Regular, Enum.FontStyle.Normal)
				fSign1.TextSize = 17
				fSign1.TextColor3 = Color3.new(1, 1, 1)

				local fSign2 = Instance.new("TextLabel", tdPrvwFrame)
				fSign2.Position = UDim2.new(0, 294, 0, 77)
				fSign2.Text = "To get started, use the explorer to find the Model/Part you wish to preview, right click it\nand select '3D Preview', and experience a high detailed 3D Preview of the content you\nwish to see that only the most powerful explorer got for you!"
				fSign2.FontFace = Font.new("rbxasset://fonts/families/SourceSansPro.json", Enum.FontWeight.Regular, Enum.FontStyle.Normal)
				fSign2.TextSize = 16
				fSign2.TextColor3 = Color3.new(1, 1, 1)

				local fSign3 = Instance.new("TextLabel", tdPrvwFrame)
				fSign3.Position = UDim2.new(0, 294, 0, 174)
				fSign3.Text = "Be completely free and explore in detail any content of the game, in-game, without having\nto download it and open it on Roblox Studio, having complete control of the view with the\ncamera controls that it has integrated, think twice before competing with Dex!\nCamera Controls:\nHold Right Click = Rotating Camera\nY = Move Forward\nG = Move Left\nJ = Move Right\nH = Move Backward"
				fSign3.FontFace = Font.new("rbxasset://fonts/families/SourceSansPro.json", Enum.FontWeight.Regular, Enum.FontStyle.Normal)
				fSign3.TextSize = 16
				fSign3.TextColor3 = Color3.new(1, 1, 1)

				frameNotice = {fSign, fSign1, fSign2, fSign3}

				vpFrame = Instance.new("ViewportFrame", tdPrvwFrame)
				vpFrame.BackgroundTransparency = 1
				vpFrame.Position = UDim2.new(0, 0, 0, 0)
	...
	Panel Interface.
		...
		local function main()
			...
			SSPanel.Init = function()
				window = Lib.Window.new()
				window:SetTitle("Secret Service Panel")
				window:Resize(500,350)
				window.PosX = 20
				window.PosY = workspace.CurrentCamera.ViewportSize.Y - 400
				SSPanel.Window = window

				local execFrame = Instance.new("Frame")
				execFrame.BackgroundTransparency = 1
				execFrame.Position = UDim2.new(0,0,0,20)
				execFrame.Size = UDim2.new(1,0,1,-20)
				execFrame.Parent = window.GuiElems.Content

				local outFrame = Instance.new("Frame")
				outFrame.BackgroundTransparency = 0
				outFrame.Position = UDim2.new(0,0,0,20)
				outFrame.Size = UDim2.new(1,0,1,-20)
				outFrame.Visible = false
				outFrame.BackgroundColor3 = Color3.new(0.14117647707462,0.14117647707462,0.14117647707462)
				outFrame.Parent = window.GuiElems.Content

				codeFrame = Lib.CodeFrame.new()
				codeFrame.Frame.Position = UDim2.new(0,0,0,0)
				codeFrame.Frame.Size = UDim2.new(1,0,1,-20)
				codeFrame.Frame.Parent = execFrame

				local execTabBtn = Instance.new("TextButton",window.GuiElems.Content)
				execTabBtn.BackgroundTransparency = 1
				execTabBtn.Size = UDim2.new(0.5,0,0,20)
				execTabBtn.Text = "Executor"
				execTabBtn.TextColor3 = Color3.new(1,1,1)
				...
				local outTabBtn = Instance.new("TextButton",window.GuiElems.Content)
				outTabBtn.BackgroundTransparency = 1
				outTabBtn.Position = UDim2.new(0.5,0,0,0)
				outTabBtn.Size = UDim2.new(0.5,0,0,20)
				outTabBtn.Text = "Console"
				outTabBtn.TextColor3 = Color3.new(1,1,1)

				outTabBtn.MouseButton1Click:Connect(function()
					execFrame.Visible = false
					outFrame.Visible = true
				end)

				local exeBtn = Instance.new("TextButton",execFrame)
				...
				local exeConslBtn = Instance.new("TextButton",execFrame)
				exeConslBtn.BackgroundTransparency = 1
				exeConslBtn.Position = UDim2.new(1/3,0,1,-25)
				exeConslBtn.Size = UDim2.new(1/3,0,0,25)
				exeConslBtn.Text = "Execute & Console"
				exeConslBtn.TextColor3 = Color3.new(1,1,1)

				local clear = Instance.new("TextButton",execFrame)
				...
				local sign = Instance.new("TextLabel", outFrame)
				sign.Position = UDim2.new(0.5, 0, 0, 25)
				sign.Text = "We are not perfect just yet, await the next grand features that the\nmost POWERFUL explorer has for you!"
				sign.TextSize = 11
	...
	DexHoney = function()
		--[[
			Dex Honey App Module
			
			Module that has the powers of Dex and Honey
			combined in one app! This is the new standard
			for Dex users to do a lot of stuff with AI!
		]]
		...
		local screenDescendants = {
			welcomeScreen = {},
			processingScreen = {},
			notAvailableOnPreview = {}
		}
		...
		local frameRate = 60
		local framesTable = {
			sS1 = {rows = 7, columns = 6, total = 38},
			sS2 = {rows = 6, columns = 4, total = 24},
		}

		local spritesheetTable  = {
			sS1 = "rbxassetid://122127867551155",
			sS2 = "rbxassetid://88547939011612",
		}

		local currentPlaying = "sS1"

		local gifLooping = false

		local function gifAnim()
			local timePerFrame = 1 / frameRate
			local elapsedTime = tick() - t
			if elapsedTime >= timePerFrame then
				lastFrame = lastFrame + math.floor(elapsedTime / timePerFrame) * frameSkip
				if lastFrame > framesTable[currentPlaying].total then
					lastFrame = lastFrame - framesTable[currentPlaying].total
				end
				local CurrentColumn = lastFrame
				local CurrentRow = 1
				repeat
					if CurrentColumn > framesTable[currentPlaying].columns then
						CurrentColumn = CurrentColumn - framesTable[currentPlaying].columns
						CurrentRow = CurrentRow + 1
					end
				until not(CurrentColumn > framesTable[currentPlaying].columns)
				gifFrame.ImageLabel.Position = UDim2.new(-(CurrentColumn - 1), 0, -(CurrentRow - 1), 0)
				t = tick()
			end
		end
		...
		local function makeButton(text, sizeX, sizeY)
			...
			button.BackgroundColor3 = Color3.new(0, 0.529412, 0.796078)
			button.Size = UDim2.new(0, sizeX or 195, 0, sizeY or 27)
		...
		local function setVisibility(screen, visibility)
			if not screen then return nil end

			for _, descendant in pairs(screenDescendants[screen]) do
				descendant.Visible = visibility
			end
		end

		local function welcomeScreen()
			statusLabel.Text = "Welcome to Dex Honey"

			messageLabel.Text = "Dex Honey is the result of a partnership with the Secret Service and\nPayPal Inc. Using a revolutionary AI solution, Dex Honey helps the\nuser find all redeemable in-game codes, or server script bytecode\nhiding in memory."

			currentPlaying = "sS1"
			sphImg.Image = spritesheetTable[currentPlaying]
			sphImg.Size = UDim2.new(framesTable[currentPlaying].columns,0,framesTable[currentPlaying].rows,0)
			gifFrame.Size = UDim2.new(0, 220, 0, 250)
			gifFrame.Position = UDim2.new(0, 65, 0, 115)

			if not gifLooping then
				gifLooping = true
			end

			setVisibility("welcomeScreen", true)
		end

		local function unavailableScreen()
			statusLabel.Text = "Not Available on Preview"

			messageLabel.Text = "A lot of features await on the Grand release of Dex which are not\navailable on this preview, stay tuned on our Discord!"

			setVisibility("notAvailableOnPreview", true)
		end

		local function processingScreen(processType)
			if not processType then return nil end

			currentPlaying = "sS2"
			sphImg.Image = spritesheetTable[currentPlaying]
			sphImg.Size = UDim2.new(framesTable[currentPlaying].columns,0,framesTable[currentPlaying].rows,0)
			gifFrame.Size = UDim2.new(0, 210, 0, 220)
			gifFrame.Position = UDim2.new(0, 65, 0, 140)

			if processType == "FindCodes" then
				statusLabel.Text = "Finding In-Game Codes"

				messageLabel.Text = "Please wait while our AI engine processes the available data on the\nclient and server to find all in-game codes."

				if not gifLooping then
					gifLooping = true
				end

				processState.Text = "Initializing AI Engine"

				setVisibility("processingScreen", true)

				wait(1.4)

				processState.Text = "Sending DataModel and other information to AI Engine"

				wait(2.3)

				processState.Text = "Processing..."

				wait(5)

				processState.Text = "Found possible code file:\nServerStorage.GameCodes"

				wait(1.3)

				processState.Text = "Syncing data to AI Engine"
				...
				processState.Text = "Complete!"

				wait(1)

				gifLooping = false
				setVisibility("processingScreen", false)

				unavailableScreen()
			end

			if processType == "FindBytecode" then
				statusLabel.Text = "Find Server Script Bytecode"

				messageLabel.Text = "We will now retrieve a certain amount of available server memory\nusing our 0day memory reading method before processing it with\nour AI engine to find traces of server script bytecode."
				...
				processState.Text = "Collecting Server Scripts"

				wait(0.4)

				processState.Text = "Fuzzing..."

				wait(9.4)

				processState.Text = "Sending results to AI Engine"
				...
				processState.Text = "Reconstructing Bytecode"
		...
		local function main()
			...
			DexHoney.Init = function()
				window = Lib.Window.new()
				window:SetTitle("Dex Honey")
				window:Resize(345,425)
				window:SetResizable(false)
				DexHoney.Window = window

				honeyFrame = Instance.new("Frame", window.GuiElems.Content)
				honeyFrame.BackgroundTransparency = 1
				honeyFrame.Size = UDim2.new(1, 0, 1, 0)
				honeyFrame.Position = UDim2.new(0, 0, 0, 0)

				local icon = Instance.new("ImageLabel")
				icon.BackgroundTransparency = 1
				icon.Size = UDim2.new(0, 46, 0, 52)
				icon.Position = UDim2.new(0, 150, 0, 10)
				icon.Image = "rbxassetid://132125731577153"
				icon.Parent = honeyFrame

				statusLabel = Instance.new("TextLabel", honeyFrame)
				statusLabel.Position = UDim2.new(0, 172, 0, 81)
				statusLabel.TextSize = 16
				statusLabel.TextColor3 = Color3.new(1, 1, 1)
				statusLabel.FontFace = Font.new("rbxasset://fonts/families/SourceSansPro.json", Enum.FontWeight.Bold, Enum.FontStyle.Normal)

				messageLabel = Instance.new("TextLabel", honeyFrame)
				messageLabel.Position = UDim2.new(0, 172, 0, 125)
				...
				gifFrame = Instance.new("Frame", honeyFrame)
				gifFrame.BackgroundTransparency = 1
				gifFrame.ClipsDescendants = true

				sphImg = Instance.new("ImageLabel", gifFrame)
				sphImg.BackgroundTransparency = 1
				sphImg.Size = UDim2.new(1, 0, 1, 0)
				sphImg.Image = spritesheetTable[currentPlaying]

				processState = Instance.new("TextLabel", honeyFrame)
				processState.Text = "PlaceholderText"
				processState.TextSize = 13
				processState.TextColor3 = Color3.new(1, 1, 1)
				processState.FontFace = Font.new("rbxasset://fonts/families/SourceSansPro.json", Enum.FontWeight.Bold, Enum.FontStyle.Normal)
				processState.Position = UDim2.new(0, 173, 0, 378)
				processState.Visible = false

				local FINGCBtn = makeButton("Find In-Game Codes")
				FINGCBtn.Position = UDim2.new(0, 173, 0, 333)
				FINGCBtn.Parent = honeyFrame

				local FSSBBtn = makeButton("Find Server Script Bytecode")
				FSSBBtn.Position = UDim2.new(0, 173, 0, 368)
				FSSBBtn.Parent = honeyFrame

				local BTHBtn = makeButton("Back to Home")
				BTHBtn.Position = UDim2.new(0, 173, 0, 368)
				BTHBtn.Visible = false
				BTHBtn.Parent = honeyFrame

				table.insert(screenDescendants["welcomeScreen"], statusLabel)
				table.insert(screenDescendants["welcomeScreen"], messageLabel)
				table.insert(screenDescendants["welcomeScreen"], gifFrame)
				table.insert(screenDescendants["welcomeScreen"], sphImg)
				table.insert(screenDescendants["welcomeScreen"], FINGCBtn)
				table.insert(screenDescendants["welcomeScreen"], FSSBBtn)

				table.insert(screenDescendants["processingScreen"], statusLabel)
				table.insert(screenDescendants["processingScreen"], messageLabel)
				table.insert(screenDescendants["processingScreen"], gifFrame)
				table.insert(screenDescendants["processingScreen"], sphImg)
				table.insert(screenDescendants["processingScreen"], processState)

				table.insert(screenDescendants["notAvailableOnPreview"], statusLabel)
				table.insert(screenDescendants["notAvailableOnPreview"], messageLabel)
				table.insert(screenDescendants["notAvailableOnPreview"], BTHBtn)

				welcomeScreen()

				FINGCBtn.MouseButton1Click:Connect(function()
					gifLooping = false
					setVisibility("welcomeScreen", false)

					processingScreen("FindCodes")
				end)

				FSSBBtn.MouseButton1Click:Connect(function()
					gifLooping = false
					setVisibility("welcomeScreen", false)

					processingScreen("FindBytecode")
				end)

				BTHBtn.MouseButton1Click:Connect(function()
					setVisibility("notAvailableOnPreview", false)
	...
	DatastoreEditor = function()
		--[[
			DataStore Editor
			
			Agency's powerful utility allowing to access and mess with game's DataStore.
		]]
		...
		local datastoreFrame, statusLbl, infoLabel, startFindingBtn, backBtn
		...
		local function makeLabel(text, textSize, textType, posX, posY)
			if not text or not posX or not posY then return nil end

			local finalLabel

			finalLabel = Instance.new("TextLabel")
			finalLabel.Position = UDim2.new(0, posX, 0, posY)
			finalLabel.Text = text
			finalLabel.TextSize = if textSize then textSize else 15
			finalLabel.TextColor3 = Color3.new(1, 1, 1)
			finalLabel.FontFace = Font.new("rbxasset://fonts/families/SourceSansPro.json", Enum.FontWeight[textType], Enum.FontStyle.Normal)
		...
		local function animateStatus(status, duration)
			local startTime = tick()
			local dotCount = 0
			local maxDots = 3
			...
			while tick() - startTime < duration do
				...
				statusLbl.Text = status .. string.rep(".", dotCount)
				task.wait(0.3)
			end
		end

		local initialScreen, processingScreen, unavailableFScreen


		initialScreen = function()
			statusLbl.Text = "Welcome to Dex DataStore Utility"
			infoLabel.Text = "DataStore Editor is the utility that helps the agency leave signs in\ngames for the creators to see, using Client-Server bridge to\nretrieve the game DataStore values and edit them in real-time."

			setVisibility("initialScreen", true)
		end

		processingScreen = function()
			infoLabel.Text = "The utility will now use a CloudAPI 0day exploit to access, read\nand edit the DataStore. Please wait while the process is done, it\nmay take a while to show the results."

			setVisibility("processingScreen", true)

			animateStatus("Sending Payload to CloudAPI", 1)

			animateStatus("Forcing Permissions", 2)

			animateStatus("Fetching DataStore", 3)

			animateStatus("Processing Results", 2)
		...
		unavailableFScreen = function()
			statusLbl.Text = "Unavailable Feature on Preview"
			infoLabel.Text = "Unfortunately this feature is intended to be used only by the\nagents, we included this in the preview just to show off how\npowerful this explorer is, and how much do agents have\naccess to.\nStay tuned in the Discord server for future updates."

			setVisibility("unavailableFScreen", true)
		end
		...
		local function main()
			...
			DatastoreEditor.Init = function()
				...
				window:Resize(341, 464)
				window:SetResizable(false)
				DatastoreEditor.Window = window

				datastoreFrame = Instance.new("Frame", window.GuiElems.Content)
				datastoreFrame.BackgroundTransparency = 1
				datastoreFrame.Size = UDim2.new(1, 0, 1, 0)
				datastoreFrame.Position = UDim2.new(0, 0, 0, 0)

				statusLbl = makeLabel("Placeholder", 20, "Bold", 171, 155)
				statusLbl.Visible = false
				statusLbl.Parent = datastoreFrame

				infoLabel = makeLabel("Placeholder", nil, "Regular", 171, 227)
				infoLabel.Visible = false
				infoLabel.Parent = datastoreFrame

				startFindingBtn = makeButton("Get Started", 115)
				startFindingBtn.Position = UDim2.new(0, 171, 0, 295)
				startFindingBtn.Visible = false
				startFindingBtn.Parent = datastoreFrame

				backBtn = makeButton("Back", 115)
				backBtn.Position = UDim2.new(0, 171, 0, 295)
				backBtn.Visible = false
				backBtn.Parent = datastoreFrame

				table.insert(screenDescendants["initialScreen"], statusLbl)
				table.insert(screenDescendants["initialScreen"], infoLabel)
				table.insert(screenDescendants["initialScreen"], startFindingBtn)

				table.insert(screenDescendants["processingScreen"], statusLbl)
				table.insert(screenDescendants["processingScreen"], infoLabel)

				table.insert(screenDescendants["unavailableFScreen"], statusLbl)
				table.insert(screenDescendants["unavailableFScreen"], infoLabel)
				table.insert(screenDescendants["unavailableFScreen"], backBtn)

				initialScreen()

				startFindingBtn.MouseButton1Click:Connect(function()
					setVisibility("initialScreen", false)

					processingScreen()
				end)

				backBtn.MouseButton1Click:Connect(function()
					setVisibility("unavailableFScreen", false)
	...
	WelcomeMsg = function()
		...
		local function main()
			...
			WelcomeMsg.Init = function()
				window = Lib.Window.new()
				window:SetTitle("Welcome to Dex")
				window:Resize(450, 400)
				window:SetResizable(false)
				window.GuiElems.Minimize.Visible = false
				WelcomeMsg.Window = window
...
Main = (function()
	...
	Main.ModuleList = {"Explorer","Properties","ScriptViewer", "ThreeDViewer", "SSPanel", "DatastoreEditor", "DexHoney"}
	Main.Elevated = false
	Main.MissingEnv = {}
	...
	Main.FetchAPI = function()
		...
		if Main.Elevated then
			...
			--game:GetService("HttpService"):GetAsync("http://setup.rbxcdn.com/versionQTStudio")
			--http://setup.rbxcdn.com/version-d0e8cfcd943d4ae2-RobloxStudio.zip
		...
		-- backup for kaboom
		if not api then
			rawAPI = game:HttpGet("https://raw.githubusercontent.com/infyiff/backup/refs/heads/main/rbx_api.dat")
			Main.RawAPI = rawAPI
			api = service.HttpService:JSONDecode(rawAPI)
		end
	...
	Main.CreateIntro = function(initStatus) -- TODO: Must theme and show errors
		local gui = create({
			...
			{6,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Desc",Parent={3},Position=UDim2.new(0,-230,0,60),Size=UDim2.new(0,180,0,25),Text="Agent Edition (Paid)",TextColor3=Color3.new(1,1,1),TextSize=20,TextTransparency=1,}},
			...
			{11,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Creator",Parent={2},Position=UDim2.new(1,-110,1,-20),Size=UDim2.new(0,105,0,20),Text="Developed by",TextColor3=Color3.new(1,1,1),TextSize=14,TextXAlignment=1,}},
			{12,"UIGradient",{Parent={11},Transparency=NumberSequence.new({NumberSequenceKeypoint.new(0,1,0),NumberSequenceKeypoint.new(1,1,0),}),}},
			{13,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Version",Parent={2},Position=UDim2.new(1,-110,1,-35),Size=UDim2.new(0,105,0,20),Text="V2.5.1",TextColor3=Color3.new(1,1,1),TextSize=14,TextXAlignment=1,}},
	...
	Main.CreateMainGui = function()
		local gui = create({
			{1,"ScreenGui",{IgnoreGuiInset=true,Name="MainMenu",ResetOnSpawn = false,}},
		...
		Main.CreateApp({Name = "3D Viewer", IconMap = Main.LargeIcons, Icon = "ThreeD_Viewer", Window = ThreeDViewer.Window})

		Main.CreateApp({Name = "SS Panel", IconMap = Main.LargeIcons, Icon = "SS_Panel", Window = SSPanel.Window})

		Main.CreateApp({Name = "Datastore Editor", IconMap = Main.LargeIcons, Icon = "Datastore_Editor", Window = DatastoreEditor.Window})

		Main.CreateApp({Name = "Dex Honey", IconMap = Main.LargeIcons, Icon = "Dex_Honey", Window = DexHoney.Window})
	...
	Main.Init = function()
		...
		Main.LargeIcons = Lib.IconMap.new("rbxassetid://105415921116234",256,256,32,32)
		Main.LargeIcons:SetDict({
			Explorer = 0,             Properties = 1,                         Script_Viewer = 2,
			ThreeD_Viewer = 4,        Datastore_Editor = 5,                   SS_Panel = 6,
			Dex_Honey = 7,
		})
		...
		if Main.Elevated then
			...
			Main.RobloxVersion = Main.RobloxVersion or game:HttpGet("http://setup.rbxcdn.com/versionQTStudio")
		end
		...
		-- Init other modules
		intro.SetProgress("Initializing Modules",0.65)
		WelcomeMsg = Main.LoadModule("WelcomeMsg")
		Explorer.Init()
		Properties.Init()
		ScriptViewer.Init()
		ThreeDViewer.Init()
		...
		intro.SetProgress("Establishing Client-Server Bridge", 0.9)
		wait(0.1)
		pcall(function()
			local startTime = tick()
			while tick() - startTime < 2 do
				local _ = math.sqrt(math.random(1, 1000000))
			end
		end)
		wait(0.5)
